...
class Usuario:
    def __init__(self, usuario, senha):
        ...
        self.senha = senha
        self.dict_user = {'1': 'Thomas', '2':'Marcos', '3':'José', '4':'Annie'}
    ...
    def autentica(self, usuario, senha):
        if self.usuario != usuario or self.senha != senha:
            self.logger.warning("Tentativa de acesso invalida")
            return False
        return True
    
    def get_usuario_by_id(self, usuario_id):
        user = self.dict_user(usuario_id, None)
        if user is None:
...
if __name__ == '__main__':
    user = Usuario("Thomas", "12345")
    user.alterar_senha('12334')
    user.autentica("Thomas", "32342")
