class Relogio:
	...
	def __init__(self, hora=0, min=0, seg=0, *args, **kwargs):
		...
		# *args no caso de uma lista, ou de não saber quantos argumentos será recebido.
		# **kwargs é no caso de ser um dicionario
		self.hora = hora
		self.min = min
		self.seg = seg
...
class Calendario_Relogio(Calendario, Relogio):
	
	def __init__(self, hora, min, seg, dia, mes, ano):
		super(Calendario_Relogio, self).__init__(hora=hora, min=min, seg=seg, dia=dia, mes=mes, ano=ano)
		
	def __str__(self):
		return super(Calendario_Relogio, self).__str__()
...
cal_real = Calendario_Relogio(23, 59, 59, 31, 12, 2015)
print(cal_real)
cal_real.tick()
print(cal_real)


print(Calendario_Relogio.mro())
